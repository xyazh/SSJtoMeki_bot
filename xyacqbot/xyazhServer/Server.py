...
class Server(BaseServer, ServerHelper):
    ...
    def checkToken(self, token: "Token|None"):
        if token is None:
            return True
        if getattr(token, "val", None) is not None:
            cookie = self.headers.get("Cookie")
            if cookie and f"token={token.val}" in cookie:
                return True
            if getattr(token, "fail_path", None):
                self.send_response(302)
                self.send_header("Location", token.fail_path)
                self.end_headers()
            else:
                self.send_error(403)
            return False
    ...
    def do_GET(self):
        self.doStart()
        try:
            fuc, token = self.app.page_manager.findPath(
                self.virtual_path, "GET")
            if fuc == None:
                self.send_error(404)
            elif not self.checkToken(token):
                return
            else:
                fuc(self)
